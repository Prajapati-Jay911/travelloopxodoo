...
export default function BillingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadInvoice = useCallback(async () => {
    setError("");
    try {
      const data = await apiFetch<InvoiceDto>(`/api/trips/${id}/invoice`);
      setInvoice(data);
    } catch (err) {
  ...
  async function markAsPaid() {
    ...
    try {
      await apiFetch(`/api/trips/${id}/invoice`, {
        method: "PATCH",
    ...
    return (
      <AppShell active="My Trips">
        <EmptyState
          ...
          body={error || "Could not load the invoice for this trip."}
          action={<Link className="font-bold text-sky-600" href={`/trips/${id}`}>Back to trip</Link>}
        />
      </AppShell>
    );
  }
  ...
  const totalSpent = invoice.grandTotal;
      ...
      <div className="mx-auto max-w-6xl space-y-8 pb-20">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4 flex-1">
            <Link href={`/trips/${id}`} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-sky-100 bg-white text-slate-500 transition hover:border-sky-300 hover:text-sky-600 shadow-sm">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm focus:border-[#ff5a3d] focus:outline-none focus:ring-1 focus:ring-[#ff5a3d]"
              />
            </div>
          </div>
          <div className="flex items-center gap-3">
             <button className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 transition hover:bg-slate-50 shadow-sm">
               <Icon name="filter" className="h-4 w-4" /> Filter
             </button>
             <button className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 transition hover:bg-slate-50 shadow-sm">
               Sort <Icon name="chevron-down" className="h-4 w-4" />
             </button>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          ...
          {/* Right Sidebar: Budget Insights & Actions */}
